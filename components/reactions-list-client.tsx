...
function matches(r: ReactionSummary, q: string): boolean {
  const haystack = [r.name, r.slug, ...r.aliases].join(" ").toLowerCase();
 ...
 */
export function ReactionsListClient({ items }: ReactionsListClientProps) {
  const inputId = useId();
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((r) => matches(r, q));
  }, [items, query]);
