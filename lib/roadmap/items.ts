...
export const ROADMAP_ITEMS: RoadmapItem[] = [
...

  {
    ...
    body: "Serve the generated spec at /api/v1/openapi.json and render it as a browsable, try-it reference (Scalar) at /reference. The hard part — an accurate spec — already ships; this is mostly wiring.",
    status: "next",
    kind: "docs",
    targetAt: NEXT_TARGET,
  ...
  },
  {
    ...
    body: "Publish /llms.txt (and a fuller /llms-full.txt) describing the API surface, endpoints, and conventions in the emerging convention for LLM crawlers. On-brand for an API whose first-class consumer is a coding agent.",
    status: "next",
    kind: "seo",
    targetAt: NEXT_TARGET,
    tags: ["llms", "easy-win"],
  },
  {
    ...
    body: "Hash each response into an ETag and honor If-None-Match so clients and CDNs can revalidate cheaply. Complements the existing Cache-Control policy and trims bandwidth before the dataset grows.",
    status: "next",
    kind: "api",
    targetAt: NEXT_TARGET,
  ...
  },
  {
    ...
    body: "A tiny liveness + dataset-version endpoint so monitors and consumers can confirm the API is up and which data snapshot they're hitting, without parsing a real payload.",
    status: "next",
    kind: "api",
    targetAt: NEXT_TARGET,
    tags: ["easy-win"],
  ...
  },
  {
    ...
    body: "Resolve many slugs in one request so consumers stop fanning out N calls. Same records, same schemas, one round trip.",
    status: "later",
    kind: "api",
  },
  {
    ...
    body: "Let callers request only the sections they need (e.g. ?fields=mechanism,interactions) to shrink payloads for the heavy drug records. Additive and backward-compatible.",
    status: "later",
    kind: "api",
  },
  {
    ...
    body: "Fill the still-empty pair-graph gap: a drug↔drug interaction network with severity-weighted edges, exposed as data and as a D3 view that reuses the mechanism-graph rendering. Educational aggregate view, not a clinical checker.",
    status: "later",
    kind: "data",
    tags: ["d3", "interactions"],
  },
  {
    id: "drug-compare-view",
    title: "Side-by-side drug comparison view",
    body: "A /compare surface that puts two or three drugs next to each other — class, mechanism, indications, identifiers, structure — for quick reference contrast. Reads straight from the existing records.",
    status: "later",
    kind: "ui",
  },
  {
    id: "publish-sdks-registries",
    title: "Publish SDKs to npm and PyPI on release",
    body: "Wire the generated TypeScript and Python clients into a release workflow that versions and publishes them automatically, so `npm i` / `pip install` track the live contract.",
    status: "later",
    kind: "devx",
    tags: ["sdk", "ci"],
  },
  ...
  },
  {
    ...
    body: "Open question: a thin GraphQL layer over the same repository for consumers who want to select exactly the fields and relations they need in one query. Worth it only if field-selection demand turns out to be real.",
    status: "exploring",
    kind: "api",
    tags: ["graphql"],
  },
  {
    ...
    body: "Turn the offline Tanimoto fingerprint index that already powers per-drug analogs into an interactive query — paste a SMILES and get the closest drugs in the dataset. Structural proximity only, never therapeutic equivalence.",
    status: "exploring",
    kind: "api",
  ...
  },
  {
    ...
    body: "Surface coarse, aggregate adverse-event report counts per drug from the openFDA FAERS endpoint, framed strictly as reference statistics with heavy caveats. Needs careful framing to stay reference-style, never signal detection.",
    status: "exploring",
    kind: "data",
    tags: ["openfda", "faers"],
  },
  {
    ...
    body: "Tag records with current FDA drug-shortage status from the openFDA shortages dataset. A genuinely useful public fact, but it updates often and would need its own refresh cadence.",
    status: "exploring",
    kind: "data",
    tags: ["openfda"],
  },
