...
export const ROADMAP_ITEMS: RoadmapItem[] = [
  ...
  // ── Later (2026-06 brainstorm intake) ────────────────────────────
  {
    ...
    title: "DailyMed SPL ingest (richer, fresher labels)",
    body: "DailyMed's daily Structured Product Label XML is the planned-but-unstarted source named in the README. Richer and fresher than the openFDA label join — structured sections, inactive ingredients, and packaging — slotting into the existing shared ingest record builder rather than forking it.",
    status: "later",
    kind: "data",
    tags: ["dailymed", "spl", "ingest"],
  },
  {
    id: "inactive-ingredient-index",
    title: "Inactive-ingredient / excipient index (allergen lookup)",
    body: "DailyMed SPL exposes inactive ingredients per product (free, public). Surfaced as a browsable entity so a reader can answer \"which formulations are lactose-free / dye-free / gluten-free\" — a reference fact, never advice. Additive entity following the standard six-step recipe; depends on the DailyMed ingest landing first.",
    status: "later",
    kind: "data",
    tags: ["excipients", "allergens", "dailymed"],
  },
  {
  ...
  {
    id: "lactmed-lactation-data",
    title: "Pregnancy & lactation reference data (NIH LactMed)",
  ...
  {
    id: "chembl-target-bioactivity",
    title: "ChEMBL drug–target bioactivity to flesh out the MoA graph",
    body: "ChEMBL (named in the README) is the free source for cited drug–target relationships. Would fill the still-thin target nodes in the mechanism-of-action graph with real, provenance-tagged bioactivity edges rather than label-derived inference alone.",
    status: "later",
    kind: "data",
    tags: ["chembl", "targets", "moa"],
  },
  {
  ...
  },
  {
    ...
    body: "Offset pagination is fine at a few thousand records but drifts under concurrent writes. Stable cursors are the right primitive before heavy bulk consumers arrive — additive alongside the existing limit/offset so nothing breaks.",
    status: "later",
    kind: "api",
    tags: ["pagination", "scale"],
  },
  {
